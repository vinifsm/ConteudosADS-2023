...
public class TelaConsultaProduto extends JDialog {
	...
	private ObjectTableModel<Produto> model = null;// modelo da JTable
	...
	private JLabel lbConsultaProdutos;

	/**
	 * @wbp.parser.constructor
	 */
	public TelaConsultaProduto(JFrame owner) {
		super(owner);
		initialize();
	}

	public TelaConsultaProduto(JDialog owner) {// construtor do formulario
		super(owner);
	...
	private void initialize() {
		...
		this.setTitle("Consulta de Produto");
	...
	private void carregaTable(List lista) {
		...
		AnnotationResolver resolver = new AnnotationResolver(Produto.class);
		// instancia o modelo da tabela, com os campos que ir� aparecer separado
		// por virgula
		model = new ObjectTableModel<Produto>(resolver, "codigo,nomeProduto,valor");
		model.setData(lista);
		...
		table.getColumnModel().getColumn(1).setPreferredWidth(493);
	...
	private JPanel getJContentPane() {
		...
		if (jContentPane == null) {
			...
			lbInformacao.setBounds(new Rectangle(10, 317, 497, 16));
			lbInformacao.setText("Enter/Dois cliques para selecionar");
			lbBusca = new JLabel();
			lbBusca.setBounds(new Rectangle(7, 81, 40, 20));
			lbBusca.setText("Buscar:");
			lbTotal = new JLabel();
			lbTotal.setHorizontalAlignment(SwingConstants.RIGHT);
			lbTotal.setBounds(new Rectangle(419, 317, 260, 16));
			lbTotal.setText("");
	...
	private JTextField getTxtBusca() {
		if (txtBusca == null) {
			...
			txtBusca.setToolTipText("Buscar produto");
			txtBusca.setBounds(new Rectangle(54, 81, 369, 20));
	...
	private JScrollPane getJScrollPane() {
		if (jScrollPane == null) {
			...
			jScrollPane.setBounds(new Rectangle(7, 119, 672, 193));
	...
	private JButton getBtnFechar() {
		if (btnFechar == null) {
			...
			btnFechar.setIcon(new ImageIcon(TelaConsultaProduto.class.getResource("/br/com/crud/img/icons8-fechar-janela-30.png")));
			btnFechar.setFont(new Font("Tahoma", Font.PLAIN, 17));
			btnFechar.setBounds(new Rectangle(529, 344, 150, 35));
			// btnFechar.setIcon(new
			// ImageIcon(TelaConsultaProduto.class.getResource("/icones/alertacancela.png")));
	...
	private JButton getBtnCad() {
		if (btnCad == null) {
			btnCad = new JButton();
			btnCad.setToolTipText("Cadastrar Produto");
			btnCad.setFont(new Font("Tahoma", Font.PLAIN, 11));
			btnCad.setBounds(new Rectangle(554, 78, 125, 27));
			// btnCad.setIcon(new
			...
			btnCad.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					txtBusca.requestFocus();
					TelaProduto tela = new TelaProduto(TelaConsultaProduto.this);
					tela.setNovo(true);
	...
	private JPanel getPainel() {
		if (painel == null) {
			...
			painel.add(getLbConsultaProdutos());
		}
		return painel;
	}
	private JLabel getLbConsultaProdutos() {
		if (lbConsultaProdutos == null) {
			lbConsultaProdutos = new JLabel("Consulta de Produtos");
			lbConsultaProdutos.setForeground(Color.WHITE);
			lbConsultaProdutos.setFont(new Font("Tahoma", Font.PLAIN, 32));
			lbConsultaProdutos.setBounds(195, 0, 344, 60);
		}
		return lbConsultaProdutos;
	}
}
