...
public class TelaConsultaVendedor extends JDialog {
	...
	private ObjectTableModel<Vendedor> model = null;// modelo da JTable
	...
	private JLabel lbConsultaVendedores;

	/**
	 * @wbp.parser.constructor
	 */
	public TelaConsultaVendedor(TelaPreVenda telaPreVenda) {
		super(telaPreVenda);
		initialize();
	}
	
	public TelaConsultaVendedor(TelaCliente telaCliente) {
		super(telaCliente);
		initialize();
	}

	public TelaConsultaVendedor() {// construtor do formulario
		super();
	...
	private void initialize() {
		...
		this.setTitle("Consulta de Vendedor");
		this.setSize(695, 440);
		// tamanho da tela
	...
	private void carregaTable(List lista) {
		...
		AnnotationResolver resolver = new AnnotationResolver(Vendedor.class);
		// instancia o modelo da tabela, com os campos que ir� aparecer separado
		// por virgula
		model = new ObjectTableModel<Vendedor>(resolver, "codigo,razaoSocial,cpf,email");
		model.setData(lista);
	...
	private JPanel getJContentPane() {
		...
		if (jContentPane == null) {
			...
			lbInformacao.setBounds(new Rectangle(10, 327, 497, 16));
			lbInformacao.setText("Enter/Dois cliques para selecionar");
			lbBusca = new JLabel();
			lbBusca.setBounds(new Rectangle(10, 79, 40, 20));
			lbBusca.setText("Buscar:");
			lbTotal = new JLabel();
			lbTotal.setHorizontalAlignment(SwingConstants.RIGHT);
			lbTotal.setBounds(new Rectangle(419, 327, 260, 16));
			lbTotal.setText("");
	...
	private JTextField getTxtBusca() {
		if (txtBusca == null) {
			...
			txtBusca.setToolTipText("Buscar funcionario");
			txtBusca.setBounds(new Rectangle(54, 79, 369, 20));
	...
	private JScrollPane getJScrollPane() {
		if (jScrollPane == null) {
			...
			jScrollPane.setBounds(new Rectangle(10, 109, 672, 193));
	...
	private JButton getBtnFechar() {
		if (btnFechar == null) {
			...
			btnFechar.setIcon(new ImageIcon(TelaConsultaVendedor.class.getResource("/br/com/crud/img/icons8-fechar-janela-30.png")));
			btnFechar.setFont(new Font("Tahoma", Font.PLAIN, 17));
			btnFechar.setBounds(new Rectangle(529, 354, 150, 35));
	...
	private JButton getBtnCad() {
		if (btnCad == null) {
			btnCad = new JButton();
			btnCad.setToolTipText("Cadastrar funcionario");
			btnCad.setFont(new Font("Tahoma", Font.PLAIN, 11));
			btnCad.setBounds(new Rectangle(559, 76, 125, 27));
			// btnCad.setIcon(new
			...
			btnCad.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					txtBusca.requestFocus();
					TelaCliente tela = new TelaCliente(TelaConsultaVendedor.this);
					tela.setNovo(true);
	...
	private JPanel getPainel() {
		if (painel == null) {
			...
			painel.add(getLbConsultaVendedores());
		}
		return painel;
	}
	private JLabel getLbConsultaVendedores() {
		if (lbConsultaVendedores == null) {
			lbConsultaVendedores = new JLabel("Consulta de Funcionarios");
			lbConsultaVendedores.setForeground(Color.WHITE);
			lbConsultaVendedores.setFont(new Font("Tahoma", Font.PLAIN, 32));
			lbConsultaVendedores.setBounds(176, 0, 357, 60);
		}
		return lbConsultaVendedores;
	}
}
